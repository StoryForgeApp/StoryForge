...
class MenuManager {
  ...
  updateRecentFiles(files: string[]) {
    ...
    const recentIndex = fileMenu.submenu.findIndex((item: any) => item.label === "Recent");

    if (recentIndex >= 0) {
      fileMenu.submenu[recentIndex] = {
        label: "Recent",
        submenu:
          recentMenu.length > 0 ? recentMenu : [{ label: "No recent files", enabled: false }],
      };
    }
...
function createMenu() {
  ...
  menu.push({
    ...
    submenu: [
      ...
      ...(!isMac ? [{ type: "separator" }, { label: "Exit", action: () => app.quit() }] : []),
    ],
  });
  ...
  handlers: {
    async showContextMenu(context: any) {
      ...
      if (context.hasSelection) {
        menu.push({ label: "Copy", action: () => win.rpc.copy() }, { type: "separator" });
      }
