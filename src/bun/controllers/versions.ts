...
export const versionController = {
  ...
  deleteVersion: async ({ version }: { version: string }): Promise<boolean> => {
    ...
    if (existsSync(versionFolder)) {
      rmSync(versionFolder, { force: true, recursive: true });
  ...
  downloadVersion: async ({ version }: { version: string }): Promise<void> => {
  ...
  getInstalledVersions: async (): Promise<{ version: string; size: number }[]> => {
    ...
    const filteredVersions = installedVersions
      .filter(
        (version) =>
          existsSync(join(versionsPath, version)) &&
          statSync(join(versionsPath, version)).isDirectory(),
      )
      .map(async (version) => ({
        version,
        size: await getDirSize(join(versionsPath, version)),
      }));
    return Promise.all(filteredVersions);
  },
